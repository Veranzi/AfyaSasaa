...
interface MapComponentProps {
  ...
  clinics: Array<{ id: number; name: string; lat: number; lng: number }>;
  onSelect: (clinic: any) => void;
}

export default function MapComponent({ userPos, clinics, onSelect }: MapComponentProps) {
  return (
    <MapContainer
      ...
      {clinics.map((clinic) => (
        ...
        >
          <Popup>
            <b>{clinic.name}</b>
            <br />
            <button onClick={() => onSelect(clinic)} className="text-blue-600 underline">
              Select
            </button>
          </Popup>
        </Marker>
